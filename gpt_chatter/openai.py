    ...
    def _part_four(self, url: str):
        ...
        if response.status_code == 302:
            ...
            try:
                ...
                # raise Exceptions.Auth0Exception(f"[OpenAI][4] Failed to parse state from response: `{response.text}`")
                state = ''
    ...
    def _part_five(self, state: str):
        ...
        if response.status_code == 200:
            ...
            raise Exceptions.Auth0Exception("[OpenAI][5] Failed to make the request, Try that again!"
                                            " Error code: " + str(response.status_code) + " Error text: " + response.text)
        ...
        if is_200:
            ...
            if access_token:
                ...
                raise Exceptions.Auth0Exception("[OpenAI][8] While most of the process was successful, "
                                                ...
                                                "This could just be OpenAI's servers being slow")
